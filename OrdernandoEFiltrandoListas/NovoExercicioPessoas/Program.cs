...
namespace NovoExercicioPessoas
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            listaDeInformacao.OrderByDescending(x => x.DataNascimento).ToList<Pessoas>().ForEach(i => Console.WriteLine($"Id: {i.Id} Nome: {i.Nome} Data Nascimento {i.DataNascimento}"));
            Console.WriteLine("-----------------------");

            listaDeInformacao.OrderBy(x => x.Nome).ToList<Pessoas>().ForEach(i => Console.WriteLine($"Id: {i.Id} Nome: {i.Nome} Data Nascimento {i.DataNascimento}"));
            Console.WriteLine("---------------------------");

            var filtroPessoas = listaDeInformacao.Where(x => x.Carteira > 500).OrderBy(i => i.Nome);


            filtroPessoas.ToList<Pessoas>().ForEach(i => Console.WriteLine($"Id {i.Id} Nome: {i.Nome} Peso {i.Carteira}"));
       ...
       // {
            //return listaDeInformacao.FindAll(x => (DateTime.Now.Year - x.DataNascimento.Year) <= 18);
        //}
    }
}
